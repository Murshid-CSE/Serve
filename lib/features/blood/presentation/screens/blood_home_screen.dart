...
class BloodHomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Blood Donation', style: TextStyle(fontWeight: FontWeight.bold)),
          leading: IconButton(
          ...
          bottom: const TabBar(
            ...
            tabs: [
              Tab(icon: Icon(Icons.search_rounded, size: 20), text: 'Discover'),
              Tab(icon: Icon(Icons.water_drop_rounded, size: 20), text: 'My Requests'),
              Tab(icon: Icon(Icons.volunteer_activism_rounded, size: 20), text: 'My Responses'),
            ],
          ),
        ),
...
// ─────────────────────────────────────────────
// Tab 1: Discover — Nearby active requests
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Tab 2: My Requests — Things I created
// ─────────────────────────────────────────────
      ...
      data: (requests) {
        if (requests.isEmpty) {
          return EmptyState(
            ...
            subtitle: 'Any blood requests you create will appear here.',
...
// ─────────────────────────────────────────────
// Tab 3: My Responses — Things I responded to
// ─────────────────────────────────────────────
class _MyResponsesTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final userResponsesAsync = ref.watch(userBloodResponsesProvider);
    final userAsync = ref.watch(currentUserProvider);
    ...
    return userResponsesAsync.when(
      data: (requests) {
        if (requests.isEmpty) {
          return const EmptyState(
            ...
            subtitle: 'When you offer to donate blood for a request, it will be tracked here.',
          );
        }
        ...
        return RefreshIndicator(
          onRefresh: () async {
            ref.invalidate(userBloodResponsesProvider);
          },
          child: ListView.builder(
            padding: const EdgeInsets.all(16),
            itemCount: requests.length,
      ...
      error: (err, stack) => ErrorState(
        message: err.toString(),
        onRetry: () => ref.invalidate(userBloodResponsesProvider),
      ),
    );
  }
}
